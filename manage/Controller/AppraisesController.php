    ...
    function admin_add() {
        ...
        if (!empty($this->data['Appraiseoption'])){
        	...
        	foreach($options as $option){
        		...
        		$option['qid'] = $this->insert_id;
        		if ($this->Appraiseoption->save($option)) {
        			
        		}
        	}        	
        }
        $options_list = array();
        $this->set('options_list', $options_list);
    }
	function admin_edit($id) {
        ...
        if (!empty($this->data['Appraiseoption'])){
        	...
        	foreach($options as $option){
        		...
        		if(empty($option['id'])){
        			$option['qid'] = $this->data['Appraise']['id']; // 当编辑时新增的选项，设置问题的id
        		}
        	...
        	$options_list  = $this->Appraiseoption->find('all',array('conditions'=>array('qid'=>$id,'deleted'=>0)));
...
//    	print_r($options);
    ...
    /*function admin_list(){
    	$this->loadModel('Appraiseoption');
    	$questions  = $this->Appraise->find('all',array(
    		'conditions' => array(
    			'deleted' => 0,
    		)
    	));
    	...
    	//echo $listhtml;
    	$this->set('question_list', $listhtml);
    }*/
