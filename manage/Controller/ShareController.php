...
class ShareController extends AppController
{
    ...
    public function admin_gen_stop_share_balance_logs()
    {
        ...
        foreach ($weshares as $item) {
            ...
            if (!empty($share_id)) {
                ...
                $use_score_fee = round(floatval($weshare_summery[$share_id]['use_score_total'] / 100), 2);
                ...
                $transaction_fee = floatval($weshare_summery[$share_id]['total_price']) - floatval($weshare_refund_map[$share_id]) - floatval($weshare_rebate_map[$share_id]['rebate_money']) + $current_share_repaid_money + $use_rebate_fee + $use_score_fee;
                ...
                $save_data[] = [
                    ...
                    'use_score_fee' => $use_score_fee, //使用积分费用
    ...
    public function admin_make_order($num = 1, $weshare_id, $sharer)
    {
        ...
        $rand_start = strtotime('-2 day');
        $rand_end = strtotime('now');

        $orders = [];
        foreach ($users as $user) {
            $order_date = $this->rand_date($rand_start, $rand_end);
            $order = $this->gen_order($weshare, $user, $weshare_products, $order_date, ORDER_STATUS_PAID);
            $orders[] = $order;
        }
    ...
    public function admin_make_unpaid_order($num = 1, $weshare_id, $sharer)
    {
        ...
        foreach ($users as $user) {
            $order_date = $this->rand_date($rand_start, $rand_end);
            $order = $this->gen_order($weshare, $user, $weshare_products, $order_date, ORDER_STATUS_WAITING_PAY);
            $orders[] = $order;
        }
