...
class ProductcatesController extends AppController{
	...
	function admin_delete($id=null)
	{
		...
		$Category = $this->{$this->modelClass}->findById($id);
		foreach($this->{$this->modelClass}->hasAndBelongsToMany as $k => $v)
		{
			if(!empty($Category[$k]))
			{
				echo 'Can\'t delete,have data under this category.';
				exit;
			}
		}
//		if(empty($id))
...
//		$this->redirect(array('action' => 'index'));
	...
	function admin_add() {
		...
		$parents = $this->{$this->modelClass}->generateTreeList();
		//$parents[''] = 'root';
	...
	function admin_edit($id = null) {
		...
		$this->set('cate_id',$this->data['Category'][0]['id']);
