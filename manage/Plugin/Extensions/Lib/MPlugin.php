		...
		foreach ($pluginPaths as $pluginPath) {
			...
			foreach ($pluginFolders[0] as $pluginFolder) {
				if (substr($pluginFolder, 0, 1) != '.') {
					if (!$this->_isMPlugin($pluginPath, $pluginFolder)) {
						continue;
					}
 ...
 */
	protected function _isMPlugin($pluginDir, $path) {
 ...
 */
	public function activate($plugin) {
		if (CakePlugin::loaded($plugin)) {
			return __d('Plugin "%s" is already active.', $plugin);
		}
		...
		if (!isset($pluginActivation) ||
			...
			if ($dependencies) {
				...
				return __d('Plugin "%s" depends on "%s" plugin.', $plugin, $missingPlugin);
			}
			return __d('Plugin "%s" could not be activated. Please, try again.', $plugin);
		}
	}
 ...
 */
	public function deactivate($plugin) {
		if (!CakePlugin::loaded($plugin)) {
			echo '-------';
			return __d('Plugin "%s" is not active.', $plugin);
		}
		$pluginActivation = $this->getActivator($plugin);
		if (!isset($pluginActivation) ||
			...
			return __d('Plugin could not be deactivated. Please, try again.');
		}
	}
 ...
 */
	public function delete($plugin) {
		if (empty($plugin)) {
			throw new InvalidArgumentException(__('Invalid plugin'));
		}
