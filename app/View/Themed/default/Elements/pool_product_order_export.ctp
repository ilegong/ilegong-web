...
$fields = array('id', 'consignee_name', 'created', 'pay_time', 'goods', 'total_all_price', 'ship_fee', 'coupon_total', 'status', 'consignee_mobilephone', 'consignee_address', 'ship_mark', 'business_remark', 'ship_type_name', 'ship_code');
$header = array('订单号', '客户姓名', '下单时间', '支付时间', '商品信息',  '总价', '运费', '使用红包', '状态', '联系电话', '收货地址', '快递方式', '备注', '快递公司', '快递单号');
...
foreach ($orders as $item) {
        ...
        foreach ($fields as $fieldName) {
            if ($fieldName == 'goods') {
                $value = get_share_order_cart_display_name($order_cart_map[$item['Order']['id']]);
            } else {
                if ($fieldName == 'status') {
                    $value = $order_status[$item['Order']['status']];
                } else if ($fieldName == 'coupon_total') {
                    $value = $item['Order']['coupon_total'] / 100.0;
                } else if ($fieldName == 'ship_mark') {
                    $value = $ship_mark[$item['Order']['ship_mark']];
                }else if($fieldName == 'ship_fee'){
                    $value = $item['Order']['ship_fee'] / 100.0;
                }else if($fieldName == 'consignee_mobilephone'){
                    $value = '手机:'.$item['Order']['consignee_mobilephone'];
                }else {
