...
function is_super_share_manager($uid) {
    ...
    if(WX_HOST==SH_SITE_HOST){
        ...
        $super_manager = [
            ...
            559795, 902131, 884183, 916301,12282,916319,916326
        ];
    }
 ...
 * 添加用户分组
 */
function wx_create_group($group_name , $logObj)
{
    if(!$group_name)
    {
        return false;
    ...
    if (!empty($access_token)) {
        $curl = curl_init();
        $options = array(
            CURLOPT_URL => 'https://api.weixin.qq.com/cgi-bin/groups/create?access_token=' . $access_token,
            CURLOPT_CUSTOMREQUEST => 'POST', // GET POST PUT PATCH DELETE HEAD OPTIONS
        );
        if (!empty($group_name)) {
            $options[CURLOPT_POSTFIELDS] = json_encode(['group' => ['name' => $group_name]]);
        }
        ...
        $output = json_decode($json, true);
        if (!empty($logObj)) {
            $logObj->log("post weixin api create group output: " . json_encode($output), LOG_DEBUG);
        }
        if ($output['errcode'] == 0) {
...
function wx_get_groups()
{
    ...
    if (!empty($access_token)) {
        $curl = curl_init();
        $options = array(
            CURLOPT_URL => 'https://api.weixin.qq.com/cgi-bin/groups/get?access_token=' . $access_token,
        ...
        $output = json_decode($json, true);
        if (!empty($logObj)) {
            $logObj->log("post weixin api get groups output: " . json_encode($output), LOG_DEBUG);
        }
        if ($output['errcode'] == 0) {
            return $output;
        }
...
function wx_get_group_id_by_open_id($openId)
{
    if(!$openId)
    {
        return false;
    ...
    if (!empty($access_token)) {
        $curl = curl_init();
        $options = array(
            CURLOPT_URL => 'https://api.weixin.qq.com/cgi-bin/groups/getid?access_token=' . $access_token,
            CURLOPT_CUSTOMREQUEST => 'POST', // GET POST PUT PATCH DELETE HEAD OPTIONS
        );

        if (!empty($openId)) {
            $options[CURLOPT_POSTFIELDS] = json_encode(['openid' => $openId]);
        }
        ...
        $output = json_decode($json, true);
        if (!empty($logObj)) {
            $logObj->log("post weixin api get group id by open id output: " . json_encode($output), LOG_DEBUG);
        }
        if ($output['errcode'] == 0) {
...
function wx_move_user_to_other_group($openId , $groupId)
{
    if((!$openId) && (!$groupId))
    {
        return false;
    ...
    if (!empty($access_token)) {
        $curl = curl_init();
        $options = array(
            CURLOPT_URL => 'https://api.weixin.qq.com/cgi-bin/groups/members/update?access_token=' . $access_token,
            CURLOPT_CUSTOMREQUEST => 'POST', // GET POST PUT PATCH DELETE HEAD OPTIONS
        );

        if (!empty($openId)) {
            $options[CURLOPT_POSTFIELDS] = json_encode(['openid' => $openId,'to_groupid'=>$groupId]);
        }
        ...
        $output = json_decode($json, true);
        if (!empty($logObj)) {
            $logObj->log("post weixin api move user to other group output: " . json_encode($output), LOG_DEBUG);
        }
        if ($output['errcode'] == 0) {
            return true;
        }
...
function wx_update_group_name($id , $name)
{
    if((!$id) && (!$name))
    {
        return false;
    ...
    if (!empty($access_token)) {
        $curl = curl_init();
        $options = array(
            CURLOPT_URL => 'https://api.weixin.qq.com/cgi-bin/groups/update/update?access_token=' . $access_token,
            CURLOPT_CUSTOMREQUEST => 'POST', // GET POST PUT PATCH DELETE HEAD OPTIONS
        );

        if (!empty($openId)) {
            $options[CURLOPT_POSTFIELDS] = json_encode(['group' => ['id' => $id, 'name' => $name]]);
        }
        ...
        $output = json_decode($json, true);
        if (!empty($logObj)) {
            $logObj->log("post weixin api update group name output: " . json_encode($output), LOG_DEBUG);
        }
        if ($output['errcode'] == 0) {
            return true;
        }
