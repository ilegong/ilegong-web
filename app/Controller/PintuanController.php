...
class PintuanController extends AppController {
    ...
    public function detail($share_id) {
        ...
        if (empty($tag_id)) {
            $tag_id = $this->PintuanHelper->get_tag_id_by_uid($uid, $share_id);
        }
        if (!empty($tag_id)) {
    ...
    public function make_order($type) {
        ...
        if ($order && $cart) {
            ...
            if ($tag_id) {
                ...
                $record = array('tag_id' => $tag_id, 'order_id' => $order_id, 'user_id' => $uid, 'created' => $now_date, 'pid' => $pintuan_conf['pid'], 'share_id' => $share_id);
