 ...
 */
	function setup(&$model, $config = array()) {
		...
		$this->_defaults['key'] = Configure::read('Security.salt');
		...
		$this->settings[$model->name]['key'] = substr($this->settings[$model->name]['key'], 0, 24);
		...
		if ($primary && $this->settings[$model->name]['autoDecrypt']) {
			...
			foreach ($results as &$result) {
				...
				foreach ($result[$model->name] as $field => &$value) {
					if (in_array($field, $this->settings[$model->name]['fields']) && !empty($value)) {
