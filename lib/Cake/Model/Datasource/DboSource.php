 ...
 */
	public function fields(Model $model, $alias = null, $fields = array(), $quote = true) {
		...
		if ($allFields) {
			...
			$fields = String1::tokenize($fields);
		}
 ...
 */
	protected function _parseKey($model, $key, $value) {
		...
		if ($bound) {
			return String1::insert($key . ' ' . trim($operator), $value);
		}
 ...
 */
	public function order($keys, $direction = 'ASC', $model = null) {
		...
		while (!empty($keys)) {
			...
			if (is_object($model) && $alias !== $model->alias && is_object($model->{$alias}) && isset($model->{$alias}) && $model->{$alias}->isVirtualField($key)) {
