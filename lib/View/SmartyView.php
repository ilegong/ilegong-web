 ...
 */
	...
	function __construct (&$controller)
	{
		...
		$this->ext= '.html';
		$this->Smarty->plugins_dir[] = VENDORS.'sysplugins'.DS;
		$this->Smarty->compile_dir = TMP.'smarty'.DS.'compile'.DS;
		$this->Smarty->cache_dir = TMP.'smarty'.DS.'cache'.DS;
		$this->Smarty->error_reporting = 'E_ALL & ~E_NOTICE'; 
		
		$this->Smarty->caching = false; // 开启缓存后，相当于生成了静态页面直接调用静态页面的内容。动态的无法显示。
		$this->Smarty->cache_lifetime = 300;
		
		$this->Smarty->default_modifiers = array('escape:"htmlall:UTF-8"'); //输出转义

       	 	$this->Smarty->debugging = true; 
       	 	
		if($this->theme)
		{
			$this->Smarty->template_dir = VIEWS.'themed'.DS.$this->theme.DS;
		}
		else
		{
 ...
 */
	function _render($___viewFn, $___data_for_view, $___play_safe = true, $loadHelpers = true)
	{
		...
		$this->Smarty->assign_by_ref('SmartyView', $this);
		//var_dump($this);
		//exit;
		return  $content = $this->Smarty->fetch($___viewFn);
	}
